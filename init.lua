...
-- Jump to the next Markdown link with Tab.
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function(args)
    ...
    vim.keymap.set('n', '<Tab>', function()
      local link = [=[\[[^]]*\]\([^)]\+\)]=]
      if vim.fn.search(link, 'W') == 0 then
        vim.fn.cursor(1, 1)
        if vim.fn.search(link, 'W') == 0 then
          vim.cmd 'normal! <Tab>'
        end
      end
    end, { buffer = args.buf, desc = 'Jump to next Markdown link' })

    vim.keymap.set('n', '<S-Tab>', function()
      local link = [=[\[[^]]*\]\([^)]\+\)]=]
      if vim.fn.search(link, 'bW') == 0 then
...
vim.keymap.set('n', '<localleader>zw', function()
  ...
  if not date then
    return vim.notify('No diary date in frontmatter', vim.log.levels.WARN)
  end
  local lines = vim.api.nvim_buf_get_lines(0, 0, -1, false)
  ...
  if not finish then
    return vim.notify('No frontmatter found', vim.log.levels.WARN)
  end
  local weather_marker
  for i = finish + 1, #lines do
    if lines[i] == '<!-- weather -->' then
      weather_marker = i
      break
    end
  end
  if not weather_marker or weather_marker == finish + 1 then
    return vim.notify('No weather marker found', vim.log.levels.WARN)
  end
  local weather = vim.fn.systemlist 'weather.sh Heppenheim'
  -- Preserve a blank line after the heading; otherwise replace the existing weather line.
  if lines[weather_marker - 1] == '' then
    vim.api.nvim_buf_set_lines(0, weather_marker - 1, weather_marker - 1, false, { weather[1] or '' })
  else
    vim.api.nvim_buf_set_lines(0, weather_marker - 2, weather_marker - 1, false, { weather[1] or '' })
  end
...
vim.keymap.set('n', '<localleader>zm', function()
  ...
  local start
  for i, line in ipairs(lines) do
    if line == '## Meetings' then
      start = i
      break
    end
  end
  if not start then
    return vim.notify('No ## Meetings heading', vim.log.levels.WARN)
  end
  ...
  if meetings[1] and meetings[1]:match '^## %[' then
    table.remove(meetings, 1)
  end
  while meetings[1] == '' do
    table.remove(meetings, 1)
  end
  local replacement = { '## Meetings', '' }
  vim.list_extend(replacement, meetings)
...
vim.keymap.set('n', '<localleader>zD', function()
  ...
  if date ~= '' then
    vim.cmd.edit(vim.fn.fnameescape(vim.fn.expand('~/zk/diary/' .. date .. '.md')))
  end
